...
- (void)makeToastActivity:(id)position {
    ...
    UIView * activityBgView=[[UIView alloc]initWithFrame:CGRectMake(0, 0, self.frame.size.width, self.frame.size.height)];
...
- (CGPoint)centerPointForPosition:(id)point withToast:(UIView *)toast {
    if([point isKindOfClass:[NSString class]]) {
        ...
        if([point caseInsensitiveCompare:@"top"] == NSOrderedSame) {
            return CGPointMake(self.bounds.size.width/2, (toast.frame.size.height / 2) + CSToastVerticalPadding-NAV_HEIGHT);
        } else if([point caseInsensitiveCompare:@"bottom"] == NSOrderedSame) {
            return CGPointMake(self.bounds.size.width/2, (self.bounds.size.height - (toast.frame.size.height / 2)) - CSToastVerticalPadding-NAV_HEIGHT);
            ...
            if ([self isKindOfClass:[UIWindow class]]) {
                ...
                return CGPointMake(self.bounds.size.width / 2, self.bounds.size.height / 2-NAV_HEIGHT);
            }
        }
    } else if ([point isKindOfClass:[NSValue class]]) {
        CGPoint center=[point CGPointValue];
        if (![self isKindOfClass:[UIWindow class]]) {
            center.y=center.y-NAV_HEIGHT;
        }
        return center;
    }
