...
+(NSString *) image2String:(UIImage *)image{
	...
	image = [self scaleImage:image toWidth:image.size.width/2 toHeight:image.size.height/2];
...
+(NSURL *)getBigUrl:(NSString *)urlStr{
    if (urlStr.length==0 || !urlStr) {
        return nil;
    }

    NSString * picUrlStr=[NSString stringWithFormat:@"%@/%@",SERVICE_URL,urlStr];
    NSURL * picUrl=[NSURL URLWithString:picUrlStr];
    return picUrl;
}
+(NSURL *)getMiniUrl:(NSString *)urlStr{
    if (urlStr.length==0 || !urlStr) {
        return nil;
    }
    NSString * picUrlStr=[NSString stringWithFormat:@"%@/%@",SERVICE_URL,urlStr];
