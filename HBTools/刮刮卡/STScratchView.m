...
#define pTouchMinX 80
#define pTouchMinY 5
#define pTouchMaxX 100
#define pTouchMaxY 42
...
- (void)setHideView:(UIView *)hideView
{
    ...
    int imageWidthInt = (int)CGImageGetWidth(hideImage);
    int imageHeightInt = (int)CGImageGetHeight(hideImage);

    
	bitmapBytesPerRow = (imageWidthInt * 4);
	bitmapByteCount = (bitmapBytesPerRow * imageHeightInt);
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (currentTouchLocation.x>pTouchMinX && currentTouchLocation.y>pTouchMinY && currentTouchLocation.x<pTouchMaxX && currentTouchLocation.y<pTouchMaxY) {
        isTrigger=YES;
    }
}
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (isTrigger) {
        if (_delegate && [_delegate respondsToSelector:@selector(scratchedTrigger)]) {
            [_delegate scratchedTrigger];
        }
    }
}
