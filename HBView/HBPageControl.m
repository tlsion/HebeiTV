...
#define DEFAULT_INDICATOR_WIDTH 12
#define DEFAULT_INDICATOR_MARGIN 5

@interface HBPageControl ()
@end

@implementation HBPageControl
{
@private
...
- (void)_initialize
{
	...
	_alignment = HBPageControlAlignmentCenter;
	_verticalAlignment = HBPageControlVerticalAlignmentMiddle;
...
//	CGContextFillRect(context, CGRectMake(left, 0, size.width, size.height));
	...
	for (int i = 0; i < _numberOfPages; i++) {
		...
		if (i == _displayedPage) {
			...
			image = [_currentPageImages objectForKey:[NSString stringWithFormat:@"%d",i]];
			if (nil == image) {
			...
			image = [_pageImages  objectForKey:[NSString stringWithFormat:@"%d",i]];
			if (nil == image) {
...
- (CGFloat)_leftOffset
{
	...
	CGFloat left = 0.0f;
	switch (_alignment) {
		case HBPageControlAlignmentCenter:
			left = CGRectGetMidX(rect) - (size.width / 2.0f);
			break;
		case HBPageControlAlignmentRight:
...
- (CGFloat)_topOffset
{
	...
	switch (_verticalAlignment) {
		case HBPageControlVerticalAlignmentMiddle:
			top = CGRectGetMidY(rect) - (_measuredIndicatorHeight / 2.0f);
			break;
		case HBPageControlVerticalAlignmentBottom:
...
- (void)_setImage:(UIImage *)image forPage:(NSInteger)pageIndex current:(BOOL)current
{
    ...
    UIImage * img=[dictionary objectForKey:[NSString stringWithFormat:@"%d",pageIndex]];
	img = image;
}
...
- (UIImage *)_imageForPage:(NSInteger)pageIndex current:(BOOL)current
{
	...
	return [dictionary objectForKey:[NSString stringWithFormat:@"%d",pageIndex]];
}
